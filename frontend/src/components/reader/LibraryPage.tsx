...
import {
  ...
  findEpubFile,
  findAudioFiles,
  findLibriVoxAudioUrls,
  fetchIllustrationUrls,
  downloadAndParseBook,
  enrichWithCovers,
  extractGutenbergId,
  fetchGutenbergIllustrations,
  getDownloadUrl,
  ...
  const openArchiveBook = useCallback(
    async (result: ArchiveSearchResult) => {
      ...
      try {
        ...
        // Run illustration extraction, audio detection, and LibriVox search
        // all concurrently with the book download.
        const illustrationPromise = fetchIllustrationUrls(
          result.identifier,
          metadata.files
        );

        // Check same archive for audio first, then search LibriVox
        const sameArchiveAudio = findAudioFiles(metadata.files).map((f) =>
          getDownloadUrl(result.identifier, f.name)
        );
        const audioPromise: Promise<string[] | null> =
          sameArchiveAudio.length > 0
            ? Promise.resolve(sameArchiveAudio)
            : findLibriVoxAudioUrls(title);

        const [epubResult, textResult, illustrationUrls, audioUrls] =
          await Promise.all([
            epubPromise,
            textPromise,
            illustrationPromise,
        ...
        // EPUB parser may have already embedded illustrations as blob: URLs
        const epubIllustrations =
          epubResult && "illustrations" in epubResult
            ? (epubResult as any).illustrations as string[] | undefined
            : undefined;

        let finalIllustrations: string[] | undefined =
          epubIllustrations?.length
            ? epubIllustrations
            : illustrationUrls.length > 0
            ? illustrationUrls
            : undefined;

        // Archive.org text-only items often lack image files. When we have no
        // illustrations yet, try fetching from Project Gutenberg's images EPUB
        // directly — the Gutenberg ID is embedded in every PG text file header.
        if (!finalIllustrations) {
          const rawText = (bookData as any).chapters
            ?.slice(0, 2)
            .map((ch: any) => ch.content ?? "")
            .join("\n") ?? "";
          const gutId = extractGutenbergId(rawText);
          if (gutId) {
            const pgIllustrations = await fetchGutenbergIllustrations(gutId);
            if (pgIllustrations.length > 0) {
              finalIllustrations = pgIllustrations;
            }
          }
        }
        ...
        const finalBook = {
          ...bookData,
          ...(finalIllustrations ? { illustrations: finalIllustrations } : {}),
