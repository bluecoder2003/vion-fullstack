...
# Microsoft Edge neural TTS voices — free, no API key, high quality.
# One default voice per supported non-English language.
_EDGE_TTS_VOICES = {
    "bn": "bn-IN-BashkarNeural",   # Bengali (India), male
...
def _generate_multilang_audio(text: str, lang: str, output_file: Path):
    """Generate speech in a non-English language using Microsoft Edge TTS.

    Called from the audiobook background task (sync context), so we bounce
    into asyncio.run() to drive the async edge-tts client.
    """
    import asyncio
    import edge_tts
    ...
    if not voice:
        raise ValueError(f"TTS not available for language: {lang}")
...
def process_audiobook_job(job_id: str, safe_book_id: str, preview_chapters: list[Chapter], voice: str = "af_heart", language: str = "en"):
    safe_voice = "".join(c if c.isalnum() or c in ("-", "_") else "_" for c in voice) or "af_heart"
    safe_lang = language if (language == "en" or language in _EDGE_TTS_VOICES) else "en"
    book_dir = OUTPUT_DIR / f"{safe_book_id}-{safe_voice}-{safe_lang}"
    book_dir.mkdir(parents=True, exist_ok=True)
    # Non-English audio is generated by edge-tts (MP3 output).
    use_edge_tts = safe_lang != "en"
    audio_ext = "mp3" if use_edge_tts else "wav"
    ...
    try:
        for index, chapter in enumerate(preview_chapters):
            ...
            if is_pride:
                chapter_filename = f"chapter-1-2.{audio_ext}"
                rel_path = f"{safe_book_id}-{safe_voice}-{safe_lang}/{chapter_filename}"
            ...
            for para_idx, para in enumerate(paragraphs):
                ...
                part_filename = f"chapter-{chapter.id}-part-{para_idx:04d}.{audio_ext}"
                output_file = book_dir / part_filename

                if not output_file.exists():
                    logger.info(f"[Job {job_id}] Synthesizing paragraph {para_idx + 1}/{len(paragraphs)} for chapter {chapter.id} ({safe_lang})...")
                    if use_edge_tts:
                        _generate_multilang_audio(para, safe_lang, output_file)
                    else:
                ...
                rel_path = f"{safe_book_id}-{safe_voice}-{safe_lang}/{part_filename}"
                if rel_path not in jobs_db[job_id]["ready_files"]:
                    jobs_db[job_id]["ready_files"].append(rel_path)
...
async def generate_audiobook(payload: AudiobookRequest, background_tasks: BackgroundTasks):
    ...
    # Resolve the requested voice + language BEFORE the dedup check.
    resolved_voice = payload.voice or "af_heart"
    resolved_lang = payload.language or "en"

    # Check if there is already an active job for this book + voice + language.
    existing_job_id = None
    for j_id, j_info in jobs_db.items():
        if (
            j_info.get("safe_book_id") == safe_book_id
            and j_info.get("voice") == resolved_voice
            and j_info.get("language") == resolved_lang
